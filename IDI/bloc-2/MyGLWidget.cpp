...
void MyGLWidget::iniCamera ()
{
	...
	Cam.psi = 0.;
	Cam.theta = float(M_PI)/6;
	Cam.right = Cam.left = Cam.bottom = Cam.top = Cam.mida = cpCont.radi;
}
...
void MyGLWidget::projectTransform()
{
	if (Cam.type == perspective)
		...
		Cam.Proj = glm::ortho(-Cam.left, Cam.right, -Cam.bottom, Cam.top, Cam.zN, Cam.zF);
	}
...
void MyGLWidget::viewTransform(){
	...
	Cam.View = glm::translate(Cam.View, -glm::vec3(0.,0.,cpCont.dist));
	Cam.View = glm::rotate(Cam.View, Cam.theta+angleY, glm::vec3(1.,0.,0.));
	Cam.View = glm::rotate(Cam.View, -Cam.psi+angleX, glm::vec3(0.,1.,0.));
	Cam.View = glm::translate(Cam.View, -Cam.VRP);
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) 
{
	...
	switch (event->key()) {
		...
		case Qt::Key_Z: { //Rotar 45 graus
			if (cpCont.alfaini > 0.5){
				cpCont.alfaini -= (M_PI/180);
				calcularFOV();
				Cam.mida *= 0.975;
				calculateWindow();
			}
		...
		case Qt::Key_X: { //Rotar 45 graus
			if (cpCont.alfaini < M_PI/2-(M_PI/90)){
				cpCont.alfaini += (M_PI/180);
				calcularFOV();
				Cam.mida *= 1.025;
				calculateWindow();
...
void MyGLWidget::mouseMoveEvent(QMouseEvent *event){
	...
	if (event->buttons() == Qt::LeftButton)
	{
		...
		{
			if ((cpCont.alfaini < M_PI/2-(M_PI/90) and (event->y()-yant) > 0) or (cpCont.alfaini > 0.5 and (event->y()-yant) < 0))
			{
				cpCont.alfaini += (event->y()-yant)*(M_PI/1440);
				Cam.mida *= (1+(event->y()-yant)*0.0025);
				calcularFOV();
				calculateWindow();
			}
		}
		...
		{
			Cam.theta += float((event->y()-yant)*2*M_PI)/alt;
			if (Cam.theta > 2*M_PI)
				Cam.theta -= 2*M_PI;
			else if (Cam.theta < 0)
				Cam.theta += 2*M_PI;
			if (Cam.theta > M_PI/2 and Cam.theta < 3*M_PI/2)
				Cam.psi += float((event->x()-xant)*2*M_PI)/ample;
			else
				Cam.psi -= float((event->x()-xant)*2*M_PI)/ample;
		}
		xant = event->x();
...
void MyGLWidget::calculateWindow(){
	if (Cam.ra >= 1){
		Cam.left = Cam.mida*Cam.ra;
		Cam.right = Cam.mida*Cam.ra;
		Cam.bottom = Cam.mida;
		Cam.top = Cam.mida;
	}
	else{
		Cam.bottom = Cam.mida/Cam.ra;
		Cam.top = Cam.mida/Cam.ra;
		Cam.left = Cam.mida;
		Cam.right = Cam.mida;
	}
}
